...
/* ---------- Скругление: правило одно на весь отчёт ----------
   Марка, ПРИЛИПАЮЩАЯ к оси, скругляется только с дальнего от оси края:
   у нуля край ровный, потому что там марка обрезана осью, а не заканчивается.
   Марка, висящая В ВОЗДУХЕ (шаг водопада, сегмент стопки, часть разложения),
   скругляется со всех сторон: она ничем не обрезана, и полуобрезанный угол
   заставляет искать причину, которой нет. */
...
function drawStackDiverge(a,w,h){
  ...
  const x0=PAD_X, plotW=w-PAD_X*2, bandW=plotW/ticks.length;
  /* Зазор между сегментами стопки: без него скругления соседних кубиков
     сливаются в один бар, и структура пропадает ровно там, где её и надо
     показать. Два пикселя на двух-трёх сегментах искажают высоту плеча
     меньше, чем толщина линии, — а состав становится виден. */
  const SEG_GAP=2;
  ...
  ticks.forEach((t,i)=>{
    ...
    up.forEach((x,k)=>{
      ...
      if(seg>0.5){
        ...
        /* Сегмент у самой оси обрезан ею — у него ровный нижний край.
           Остальные висят в воздухе и скруглены со всех сторон. */
        s+=(acc<0.01
             ? barUp(cx-bw/2,y,bw,seg,x.color,' class="bar up" data-s="'+x.sid+'" style="animation-delay:'+(i*26)+'ms"')
             : barFloat(cx-bw/2,y,bw,seg,x.color,' class="bar up" data-s="'+x.sid+'" style="animation-delay:'+(i*26)+'ms"'));
        acc+=seg+SEG_GAP;
      }
    });
    const topUp=acc>0?acc-SEG_GAP:0;
    s+='</g>';
    ...
    dn.forEach((x,k)=>{
      ...
      if(seg>0.5){
        ...
        s+=(acc<0.01
             ...
             : barFloat(cx-bw/2,y,bw,seg,x.color,' class="bar dn" data-s="'+x.sid+'" style="animation-delay:'+(i*26)+'ms"'));
        acc+=seg+SEG_GAP;
      }
    });
    const botDn=acc>0?acc-SEG_GAP:0;
    s+='</g>';
    /* Итог прихода стоит НАД стопкой, итог ухода — ПОД ней: число уходит
       в ту же сторону, в какую растёт плечо. */
    if(upT[i]>0)s+=txt(cx,zero-topUp-VAL_DY,fv(o,upT[i]),valOpt({delay:240+i*26}));
    if(dnT[i]>0)s+=txt(cx,Math.min(zero+botDn+VAL_DY+4,bot-2),fv(o,dnT[i]),valOpt({delay:240+i*26}));
  });
  return svg(w,h,s);
}
/* ============================================================================
