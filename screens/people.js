/* ============================================================================
   screens/people.js — список сотрудников. Неймспейс: PXSCREEN.people
   ...
   намеренно: полка слева одна на весь отчёт, и переключение режима не
   должно менять выборку.

   Отдельной вкладки у списка нет: он второй режим панели «Движение
   персонала» на главном экране. Пока это была вкладка, клик по числу
   уносил читателя в другой конец отчёта — он не успевал понять, что
   вообще произошло, и терял из виду таблицу, из которой пришёл.
   Теперь деталка раскрывается на месте сводной, и возврат — тот же
   переключатель, которым ушёл.

   body() отдаёт содержимое без панели: панель вокруг рисует тот, кто
   список показывает.
   ========================================================================== */
(function(){
'use strict';
...
/* Подсказка «как читать» вместо простыни под таблицей: событие в списке
   считается по паре «человек × продукт», и это единственное, что здесь
   действительно надо объяснить. */
const INFO={title:'Как читать: список сотрудников',
  text:'Поимённо те, кто стоит за числами сводной. Список наследует все фильтры отчёта — полку слева, период, режим — и добавляет к ним событие за период и поиск.',
  note:['События считаются по паре «человек × продукт»: один человек может и прийти на один продукт, и уйти с другого в один месяц. Поэтому число событий сходится со строками сводной по продукту, а не с её итогом.',
        'Столбец Σ — сумма аллокаций человека по всем продуктам среза. Восклицательный знак значит, что сумма не равна ста процентам.',
...
/* Панель вокруг списка — только для тестов и на случай, если список
   когда-нибудь понадобится сам по себе. Экран его так не показывает. */
function render(st){
  return U.panel({title:'Сотрудники'+U.info(INFO),
    sub:'все фильтры отчёта действуют',body:body(st),cls:'p-people'});
}

window.PXSCREEN=window.PXSCREEN||{};
window.PXSCREEN.people={render,body,filtered,match,FILTERS,INFO};
})();
